...
#else
#include <GxGDEW042T2/GxGDEW042T2.h>        // 4.2" b/w Don't forget to change Reset Pin settings in GxDEW042xxx.cpp
#endif
...
#define B_CHARGING_PIN     13   // PD5 Led Output BQ21040 - pulled low when charging
...
#define DEF_LOWBAT_VOLTAGE  34
#define DEF_CRITBAT_VOLTAGE 33
...
volatile bool isr_chrg_state = false;
/**
...
#ifdef BATTERY_MODE
typedef IrqExternalBatt<A0, 10, 59> BatterySensorType; // Batterymeasurement under real load conditions 
...
class Hal: public BaseHal {
  public:
    void init(const HMID& id) {
      BaseHal::init(id);
#ifdef BATTERY_MODE
      battery.init(seconds2ticks(12UL * 60 * 60), sysclock); //battery measure every 12 hours
...
#ifdef LIPO
// Beide Led`s einschalten während Ladevorgang
void charger_status() {
  isr_chrg_state = digitalRead(B_CHARGING_PIN);
  digitalWrite(LED_PIN_1, !isr_chrg_state);
  digitalWrite(LED_PIN_2, !isr_chrg_state);
}
#endif
...
void setup () {
#ifdef LIPO
  pinMode(B_CHARGING_PIN, INPUT_PULLUP);
  enableInterrupt(B_CHARGING_PIN,charger_status,CHANGE);
#endif
  runSetup = true;
  for (int i = 0; i < DISPLAY_LINES; i++) {
  ...
  DPRINT("Battery voltage = ");DDECLN(hal.battery.voltageHighRes()); //Einmalige Ausgabe der Batteriespannung im seriellen Monitor bei Programmstart
  sdev.initDone();
  DDEVINFO(sdev);
